...
public class AgentSession {
    ...
    private String buildSummary(Plan plan) {
        ...
        try {
            ...
            int done = plan.completedCount();
            int total = plan.size();
            int failed = plan.failedCount();
            String factual = done + " of " + total + " steps completed";
            if (failed > 0) factual += ", " + failed + " failed";

            put(messages, "system",
                "You are reporting the outcome of an autonomous agent run. " +
                "Write 2-4 plain sentences. " +
                "Base your report STRICTLY on the transcript below. " +
                "Do not claim success if any step shows FAILED. " +
                "Start your report with this exact factual line: \"" + factual + ".\" " +
                "Then add one or two sentences explaining the outcome. " +
                "No markdown. No invented results.");
