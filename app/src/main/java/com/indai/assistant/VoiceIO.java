...
public class VoiceIO implements TextToSpeech.OnInitListener {
    ...
    private static final long SILENCE_LEN_MS = 6000L;
    private static final long POSSIBLY_SILENCE_LEN_MS = 3500L;
    private static final long MIN_LEN_MS = 1500L;
    private static final int MAX_AUTO_RETRIES = 1;

    // TTS: cap to avoid long boring reads
    private static final int MAX_SPEAK_CHARS = 400;
    ...
    private boolean chimeEnabled = false;   // OFF by default
    private float speechRate = 1.05f;       // slightly faster than default
    private float speechPitch = 0.95f;      // slightly lower, less shrill
    ...
    private void startListeningInternal(boolean isRetry) {
        ...
        if (!SpeechRecognizer.isRecognitionAvailable(activity)) {
            listener.onRecognizeError(
                    "No speech recognition service found.");
            return;
        }
        ...
        // Chime only if explicitly enabled
        if (chimeEnabled && !isRetry) playWakeChime();
    ...
    public void speak(String text) {
        ...
        // Don't read walls of text — cap and signal more
        if (clean.length() > MAX_SPEAK_CHARS) {
            int cut = clean.lastIndexOf('.', MAX_SPEAK_CHARS);
            if (cut > 100) clean = clean.substring(0, cut + 1);
            else clean = clean.substring(0, MAX_SPEAK_CHARS) + "...";
        }
    ...
    public void speakShort(String text) {
        ...
        if (clean.trim().isEmpty()) return;
        if (clean.length() > MAX_SPEAK_CHARS) {
            clean = clean.substring(0, MAX_SPEAK_CHARS) + "...";
        }
        utteranceCounter++;
        lastUtteranceId = "indai-" + utteranceCounter;
        speaking = true;
    ...
    private void playWakeChime() {
        if (tone == null) return;
        try { tone.startTone(ToneGenerator.TONE_PROP_BEEP, 80); } catch (Exception ignored) {}
    }
    ...
    private static boolean isCancelPhrase(String text) {
        ...
        for (String w : CANCEL_WORDS) if (t.equals(w)) return true;
    ...
    private final RecognitionListener recognitionListener = new RecognitionListener() {
        ...
        @Override public void onBeginningOfSpeech() { autoRetriesLeft = MAX_AUTO_RETRIES; }
        ...
        public void onError(int error) {
            ...
            if (error == SpeechRecognizer.ERROR_SPEECH_TIMEOUT
                ...
                if (autoRetriesLeft > 0 && listener != null) {
                    autoRetriesLeft--;
                    activity.runOnUiThread(new Runnable() {
                        @Override public void run() { startListeningInternal(true); }
                    });
                    return;
                }
