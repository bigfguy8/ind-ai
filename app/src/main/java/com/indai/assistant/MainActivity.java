...
public class MainActivity extends Activity {
    ...
    private boolean lastSpokenWasError = false;
    ...
    private final VoiceIO.Listener voiceListener = new VoiceIO.Listener() {
        ...
        @Override public void onSpeechDone() {
            main.post(new Runnable() {
                @Override public void run() {
                    speaking = false;
                    if (lastSpokenWasError) {
                        lastSpokenWasError = false;
                        return;
                    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Accessibility onboarding — after setup, prompt once
        main.postDelayed(new Runnable() {
            @Override public void run() {
                if (!FirstRunSetup.needsSetup(config)
                        && AccessibilityOnboarding.shouldPrompt(MainActivity.this)) {
                    AccessibilityOnboarding.show(MainActivity.this);
                }
            }
        }, 1500);
    ...
    private void buildInterface() {
        ...
        header.addView(spacer(Theme.S1));
        header.addView(iconButton(R.drawable.ic_swap, v -> showQuickSwitch()));
        ...
        bottomNavView = BottomNav.build(this, BottomNav.TAB_CHAT);
        column.addView(bottomNavView);

        setContentView(root);
        installKeyboardListener(root);
    }

    private void installKeyboardListener(final View root) {
        root.getViewTreeObserver().addOnGlobalLayoutListener(
            ...
            @Override public void onGlobalLayout() {
                ...
                int threshold = screenH / 6;
                boolean keyboardOpen = keypadH > threshold;
                if (bottomNavView != null) {
                    int want = keyboardOpen ? View.GONE : View.VISIBLE;
                    if (bottomNavView.getVisibility() != want) {
                        bottomNavView.setVisibility(want);
                    }
                }
            }
        });
    ...
    private void toggleSpeaker() {
        ...
        if (!speakerOn) {
            voice.stopSpeaking();
            applySystemPrompt(false);
            Toast.makeText(this, "Voice off — full replies", Toast.LENGTH_SHORT).show();
        } else {
            applySystemPrompt(false);
            Toast.makeText(this, "Voice on — concise replies", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void updateMicButton() {
        ...
        if (listening) {
            ...
            // ALWAYS reset hint when not listening — no matter what
            input.setHint("Ask Ind AI...");
        }
    }
    ...
    protected void onResume() {
        super.onResume();
        // Safety: clear stuck listening state when app returns to foreground
        if (listening && (voice == null || !voice.isListening())) {
            listening = false;
            updateMicButton();
        }
    }
    ...
    private void sendText(String text) {
        if (text != null && text.trim().equalsIgnoreCase(".diag-all")) {
            appendMessage(".diag-all", Sender.USER);
            setSending(false);
            new Thread(new Runnable() {
                @Override public void run() {
                    final String report = diagnoseAllProviders();
                    main.post(new Runnable() {
                        @Override public void run() {
                            appendMessage(report, Sender.TOOL);
                            setSending(true);
                        }
                    });
                }
        ...
        if (text != null && text.trim().equalsIgnoreCase(".mic")) {
            appendMessage(".mic", Sender.USER);
            setSending(false);
            final String report = diagnoseMic();
            appendMessage(report, Sender.TOOL);
            setSending(true);
            return;
        }
        if (text != null && text.trim().equalsIgnoreCase(".diag")) {
            ...
            new Thread(new Runnable() {
                @Override public void run() {
                    final String report = ApiDiagnostic.test(config);
                    main.post(new Runnable() {
                        @Override public void run() {
                            appendMessage(report, Sender.TOOL);
                            setSending(true);
                ...
                main.post(new Runnable() {
                    @Override public void run() {
                        ...
                        if (speakerOn && !parsed.cleanedText.isEmpty()) {
                            speaking = true;
                            lastSpokenWasError = parsed.cleanedText.startsWith("Error:")
                                    || parsed.cleanedText.startsWith("!");
    ...
    private String diagnoseMic() {
        StringBuilder sb = new StringBuilder();
        sb.append("MIC DIAGNOSTIC\n");
        sb.append("─────────────\n\n");

        // 1. Permission
        boolean perm = checkSelfPermission(android.Manifest.permission.RECORD_AUDIO)
                == PackageManager.PERMISSION_GRANTED;
        ...
        try {
            available = android.speech.SpeechRecognizer.isRecognitionAvailable(this);
        } catch (Exception e) {
            sb.append("Recognizer check failed: ").append(e.getMessage()).append("\n");
        }
        sb.append("SpeechRecognizer available: ")
          .append(available ? "YES ✓" : "NO ✗").append("\n");

        // 3. List installed recognition services via PackageManager
        try {
            android.content.Intent intent = new android.content.Intent(
                    android.speech.RecognitionService.SERVICE_INTERFACE);
            java.util.List<android.content.pm.ResolveInfo> services =
                    getPackageManager().queryIntentServices(intent, 0);
            sb.append("Recognition services installed: ")
              .append(services == null ? 0 : services.size()).append("\n");
            if (services != null && !services.isEmpty()) {
                ...
                sb.append("  ⚠ No recognition service installed!\n")
                  .append("  Install the Google app (com.google.android.googlequicksearchbox)\n")
                  .append("  from your app store.\n");
            }
        } catch (Exception e) {
            sb.append("Package query failed: ").append(e.getMessage()).append("\n");
        }

        // 4. VoiceIO state
        sb.append("\nVoiceIO state:\n");
        try {
            sb.append("  TTS ready: ").append(voice.isTtsReady()).append("\n");
            sb.append("  Currently listening: ").append(voice.isListening()).append("\n");
            sb.append("  Currently speaking: ").append(voice.isSpeaking()).append("\n");
        } catch (Exception e) {
            sb.append("  state check failed: ").append(e.getMessage()).append("\n");
        }
        ...
        try {
            ...
            if (am != null) {
                ...
                sb.append("  Music volume: ").append(am.getStreamVolume(
                        android.media.AudioManager.STREAM_MUSIC)).append("/")
                  .append(am.getStreamMaxVolume(android.media.AudioManager.STREAM_MUSIC))
                  .append("\n");
                sb.append("  Mic muted: ").append(am.isMicrophoneMute()).append("\n");
            }
        } catch (Exception e) {
            sb.append("AudioManager failed: ").append(e.getMessage()).append("\n");
        }

        sb.append("\nACTIONS:\n");
        if (!perm) sb.append("→ Grant mic permission in phone Settings\n");
        if (!available) sb.append("→ Install Google app for speech recognition\n");
        sb.append("→ Now try: tap mic and speak\n");
        sb.append("→ After mic test, run .mic again to see 'listening' state\n");

        return sb.toString();
    }

    private String diagnoseAllProviders() {
        StringBuilder sb = new StringBuilder();
        sb.append("PROVIDER ENDPOINT CHECK\n");
        sb.append("═════════════════════════\n\n");

        for (Providers.Provider p : Providers.ALL) {
            // Only test providers we have keys for
            String testKey = null;
            if (p.name.contains("Agnes") && config.apiKey != null
                    && config.apiKey.startsWith("sk-")) {
                // Only if user's key looks like Agnes
                if (config.endpoint != null
                        && config.endpoint.contains("agnes-ai.com")) {
                    testKey = config.apiKey;
                }
            } else if (p.name.equals(config.primaryProviderName)) {
                testKey = config.apiKey;
            }

            sb.append(p.name).append("\n");
            sb.append("  ").append(p.endpoint).append("\n");

            if (testKey == null || testKey.isEmpty()) {
                sb.append("  Status: skipped (no key)\n\n");
                continue;
            }

            ApiConfig temp = new ApiConfig(p.endpoint, testKey,
                    p.defaultModel, config.systemPrompt, "");
            String result = ApiDiagnostic.test(temp);

            // Extract key line
            String status = "(unknown)";
            for (String line : result.split("\n")) {
                if (line.startsWith("HTTP:") || line.startsWith("Exception")) {
                    status = line;
                    break;
                }
            }
        ...
        sb.append("Use Settings → PRIMARY to switch providers.\n");
        return sb.toString();
    }

    private void showQuickSwitch() {
        QuickSwitch.show(this, config, new QuickSwitch.Callback() {
            @Override public void onConfigChanged(final ApiConfig newConfig) {
                config = newConfig;
                keyStore.save(config);
                applySystemPrompt(false);
                Toast.makeText(MainActivity.this,
                        "Switched to " + config.model, Toast.LENGTH_SHORT).show();
            }
    ...
    private void showModelPicker() {
        ModelPicker.show(this, config, new ModelPicker.OnPick() {
    ...
    private void showSettings() {
        ...
        // ---- Current model row ----
        TextView modelRow = new TextView(this);
        modelRow.setText("CURRENT MODEL\n" + config.model);
        modelRow.setTextColor(Theme.PRIMARY);
        modelRow.setTextSize(Theme.T_BODY);
        modelRow.setTypeface(Typeface.create("sans-serif-medium", Typeface.NORMAL));
        int mrpH = Theme.dp(this, Theme.S4);
        int mrpV = Theme.dp(this, Theme.S3);
        modelRow.setPadding(mrpH, mrpV, mrpH, mrpV);
        modelRow.setBackground(Drawables.outlined(this,
                Theme.SURFACE, Theme.SURFACE_STROKE, Theme.R_MD, 1f));
        LinearLayout.LayoutParams mrLp = new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT);
        mrLp.bottomMargin = Theme.dp(this, Theme.S3);
        modelRow.setLayoutParams(mrLp);
        modelRow.setOnClickListener(v -> {
            v.performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP);
            ModelPicker.show(MainActivity.this, config, new ModelPicker.OnPick() {
                @Override public void onPick(final String model) {
        ...
        addProviderRow(layout, "SCREEN CONTROL  (optional)",
            ...
            @Override public void run() {
                ...
                // Show the same friendly modal
                AccessibilityOnboarding.resetAsked(MainActivity.this);
                AccessibilityOnboarding.show(MainActivity.this);
            }
        });
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Save", (d, w) -> {
                    ...
                    config = new ApiConfig(
                            config.endpoint, config.apiKey, config.model,
                            config.systemPrompt, // unchanged — set via res/raw
                            config.visionModel,
                            config.fallbackEndpoint, config.fallbackApiKey, config.fallbackModel,
                            config.visionEndpoint, config.visionApiKey,
                            config.primaryProviderName,
