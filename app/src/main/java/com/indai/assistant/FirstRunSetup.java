...
public final class FirstRunSetup {
    ...
    public static boolean needsSetup(ApiConfig config) {
        // Only show wizard if NO provider has been explicitly chosen.
        // Provider name is set when user taps a provider in the picker.
        if (config.primaryProviderName != null && !config.primaryProviderName.isEmpty()) {
            return false;
        }
        // Legacy: if endpoint is empty or still the BuildConfig default blank, show wizard
        return config.endpoint == null || config.endpoint.trim().isEmpty();
    }
